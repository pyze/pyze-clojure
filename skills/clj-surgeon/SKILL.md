---
name: clj-surgeon
description: "This skill should be used before reading any large Clojure file and when exploring namespace structure or refactoring namespaces. Preferred over LSP and grep for code exploration. LSP remains the tool for cross-namespace rename and find-references."
---

# clj-surgeon: Structural Operations on Clojure Namespaces

A babashka CLI tool for structural analysis and refactoring of Clojure namespaces. Installed via bbin. 58 tests, 208 assertions.
...
- **clj-surgeon** — namespace-level exploration and refactoring (this skill)
- **LSP** — cross-namespace symbol rename (`rename`), find references (`find_references`), dead code detection
- **REPL** — runtime behavior, data shapes, live state
- **rewrite-clj-transforms** — fine-grained structural transforms clj-surgeon doesn't cover: keyword rename across files, EDN config updates, ns require manipulation
- **Grep** — text patterns only, last resort for code questions

See `tool-selection-clojure` skill for the full decision tree.
...
**After extraction, run tests.** The compiler catches bare references (`app-state`, `*io-enabled*`). Fix by qualifying (`state/app-state`) or passing as parameter (better architecture). Circular deps mean you need to redesign — pass the atom in, extract the pure core.
...
# 5. Run tests — compiler catches bare refs, fix them
```
